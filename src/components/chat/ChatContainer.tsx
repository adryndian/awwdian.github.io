...
export function ChatContainer({ userId, initialModel }: ChatContainerProps) {
  ...
  const [selectedModel, setSelectedModel] = useState<ModelId>(
    isValidModelId(initialModel || '') ? (initialModel as ModelId) : DEFAULT_MODEL
  );
  ...
  const handleSendMessage = useCallback(async (content: string) => {
    ...
    try {
      ...
      if (!data.message) throw new Error('Model mengembalikan respons kosong.');
  ...
  const handleModelChange = useCallback((newModel: string) => {
    if (isValidModelId(newModel)) setSelectedModel(newModel);
  }, []);

  const handleClearChat = useCallback(() => {
    setMessages([]); setError(null); setCost(null); setAiStatus('idle');
  }, []);
  ...
  return (
    <div className="flex flex-col h-screen bg-gray-950 text-white">
      <div className="flex items-center justify-between px-6 py-4 border-b border-white/10 bg-gray-900/50">
        ...
        <div className="flex items-center gap-3">
          <select value={selectedModel} onChange={(e) => handleModelChange(e.target.value)} disabled={isLoading}
            className="px-3 py-1.5 text-sm rounded-lg bg-white/5 border border-white/10 text-white hover:bg-white/10 transition-colors disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-orange-500/50">
            {allModels.map((model) => (
              <option key={model.id} value={model.id} className="bg-gray-900">{model.name} ({model.costLevel} cost)</option>
            ))}
          </select>
          <button onClick={handleClearChat} className="px-3 py-1.5 text-sm rounded-lg text-white/50 hover:text-white hover:bg-white/10 transition-colors">Clear</button>
        </div>
      </div>
        ...
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center space-y-4">
            ...
            <div className="flex flex-wrap justify-center gap-2 mt-2">
              {allModels.map((model) => (
                <span key={model.id} className="rounded-full bg-white/5 border border-white/10 px-3 py-1 text-xs text-white/50">{model.name}</span>
              ))}
            </div>
          </div>
        )}
      ...
      <div className="px-4 py-4 border-t border-white/10 bg-gray-900/30">
        <div className="flex gap-3 items-end max-w-4xl mx-auto">
          <textarea ref={inputRef} value={inputValue} onChange={(e) => setInputValue(e.target.value)} onKeyDown={handleKeyDown} disabled={isLoading}
            placeholder={`Chat dengan ${currentModel?.name || 'AI'}... (Enter kirim)`} rows={1}
            className="flex-1 px-4 py-3 text-sm rounded-xl bg-white/5 border border-white/10 text-white placeholder:text-white/30 resize-none overflow-hidden focus:outline-none focus:ring-2 focus:ring-orange-500/50 disabled:opacity-50 max-h-40 transition-colors"
